...
class TestStaffCompleteTrek(unittest.TestCase):
    def setUp(self):
    ...
    def test_staff_can_mark_assigned_trek_completed(self):
        trek = Trek(
        ...
        db.session.add(trek)
        db.session.add(Booking(user_id=1, trek_id=1, status="booked", payment_status="paid"))
        db.session.commit()

        response = self.client.put(
            f"/api/staff/treks/{trek.id}/complete",
    ...
    def test_completed_trek_cannot_be_completed_again(self):
        trek = Trek(
        ...
        self.assertIn("already completed", payload["message"])
