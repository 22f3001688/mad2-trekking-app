    ...
    <div class="trek-create-page">
      <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
        <div>
          ...
          <h1 class="h3 mb-1">Edit Trek</h1>
          <p class="text-muted mb-0">Update an existing trek and keep availability aligned with bookings.</p>
        </div>
      ...
      <div class="card shadow-sm border-0">
        <div class="card-body p-4 p-lg-5">
          <div v-if="loading" class="text-center py-5">
            ...
            <p class="text-muted mb-0">Loading trek details…</p>
          </div>

          <div v-else-if="serverError" class="alert alert-danger" role="alert">
          ...
          <form v-else class="needs-validation" novalidate @submit.prevent="submitForm">
            <div class="row g-4">
              ...
              <div class="col-md-6">
                ...
                <select v-model="form.status" class="form-select" :class="{ 'is-invalid': validationErrors.status }" required>
...
export default {
  ...
  methods: {
    async fetchTrek() {
      ...
      try {
        const response = await http.get(`/admin/treks/${this.$route.params.id}`)
        if (response.data?.success) {
          ...
          this.serverError = response.data?.message || 'Unable to load trek details.'
        }
      } catch (err) {
        this.serverError = err.response?.data?.message || 'Unable to load trek details.'
      } finally {
        this.loading = false
      }
    },
    async submitForm() {
      ...
      try {
        const response = await http.put(`/admin/treks/${this.$route.params.id}`, this.form)
        if (response.data?.success) {
          this.$router.push({ name: 'AdminTrekIndex' })
        } else {
          this.serverError = response.data?.message || 'Unable to update trek.'
        }
      } catch (err) {
        if (err.response?.status === 400 && err.response?.data?.message) {
          this.serverError = err.response.data.message
        } else {
          this.serverError = err.response?.data?.message || 'Unable to update trek.'
        }
      } finally {
